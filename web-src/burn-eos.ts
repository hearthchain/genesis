...
(function () {
  ...
  const fmtA = (units: string) => fmtUnits(units, 4); // A and legacy EOS carry 4 decimals
  ...
  el("preview-form").addEventListener("submit", async (e) => {
    e.preventDefault();
    const account = (el("preview-input") as HTMLInputElement).value.trim();
    if (!account) return;
    const err = el("preview-error");
    err.hidden = true;
    el("preview-result").hidden = true;
    (el("preview-go") as HTMLButtonElement).disabled = true;
    const res = await apiGet<PreviewResponse>("/api/preview/eos/" + encodeURIComponent(account));
  ...
  function bindCopy(buttonId: string, sourceId: string, doneId: string) {
  ...
  el("memo-form").addEventListener("submit", (e) => {
    e.preventDefault();
    const account = (el("memo-account") as HTMLInputElement).value.trim();
    const hearth = (el("memo-hearth") as HTMLInputElement).value.trim();
  ...
  if (params.get("hearth")) (el("memo-hearth") as HTMLInputElement).value = params.get("hearth")!;
})();
