...
(function () {
    ...
    async function initBind() {
        ...
        if (params.get("hearth"))
            el("bind-hearth").value = params.get("hearth");
        ...
        try {
            ...
            if (!source)
                throw new Error("no active account in Keeper");
    ...
    el("burn-keeper").addEventListener("click", async () => {
        ...
        try {
            ...
            let txId = "";
            try {
                txId = JSON.parse(published).id || "";
            }
            catch { /* Keeper answered non-JSON: the link below is just skipped */ }
    ...
    el("bind-sign").addEventListener("click", async () => {
        ...
        if (!hearth) {
            showBind("err", "Enter the Hearth address first.");
            return;
        }
