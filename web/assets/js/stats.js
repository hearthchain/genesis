...
(function () {
    ...
    const set = (id, text) => { const n = document.getElementById(id); if (n)
        n.textContent = text; };
    async function refresh() {
        ...
        const pendingParts = [];
        if (waves.pendingBaseUnits !== "0")
            pendingParts.push("+" + fmtWaves(waves.pendingBaseUnits) + " WAVES");
        if (eos.pendingBaseUnits !== "0")
            pendingParts.push("+" + fmtUnits(eos.pendingBaseUnits, 4) + " A");
